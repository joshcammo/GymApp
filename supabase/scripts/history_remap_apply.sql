...
--
-- Run ONCE in the Supabase SQL editor. The whole remap is a single
-- DO block — one statement, one session, atomic — because the SQL
-- editor's pooled backend doesn't guarantee consecutive statements
-- share a session (a temp table created in one statement can be
-- invisible to the next; the first version of this script failed
-- exactly that way, before touching any data).
--
-- A pre-flight assertion inside the block aborts everything if any
-- def_id no longer matches the catalog name that was reviewed, so
-- a drifted catalog can't silently mislink history.
--
-- Not idempotent by design: a second run fails loudly on the
-- custom-exercise unique index rather than duplicating anything.
-- ================================================================
...
-- ── Verify (separate statements are fine here — read-only) ──────
-- Expect 0:
